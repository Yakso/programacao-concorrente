...
class LE:
	def __init__(self):
		self.escritores = 0
		self.p_escritores = 0 #Potenciais Escritores
		self.leitores = 0

	def leitorBloqueado(self,id):
		'''Recebe o id do leitor. Verifica se a decisao de bloqueio esta correta.'''
		if(self.escritores==0) and (self.p_escritores==0):
			print("ERRO: leitor " + str(id) + " bloqueado quando nao ha escritores nem potenciais escritores!")
	...
	def leitorLendo(self,id):
		...
		if(self.p_escritores>0):
			print("ERRO: leitor " + str(id) + " esta lendo quando ha potenciais escritores!")
		self.leitores+=1
