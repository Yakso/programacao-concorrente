...
int nthreads;
//variaveis que representam a quantidade de leitores e escritores
int contador_leitores = 0;
int contador_escritores = 0;
...
void* tarefa(void* arg) {
   ...
   for(long int i=id; i<QTDE_OPS; i+=nthreads) {
      op = rand() % 100;
      if(op<98) {
	 InicioLeitura(&contador_leitores, &contador_escritores, &mutex, &cond_leit); /* lock de LEITURA */    
         Member(i%MAX_VALUE, head_p);   /* Ignore return value */
	 FimLeitura(&contador_leitores, &mutex, &cond_escr);     
	 read++;
      } else if(98<=op && op<99) {
	 InicioEscrita(&contador_escritores, &contador_leitores, &mutex, &cond_escr); /* lock de ESCRITA */    
         Insert(i%MAX_VALUE, &head_p);  /* Ignore return value */
	 FimEscrita(&contador_escritores, &mutex, &cond_escr, &cond_leit);     
	 in++;
      } else if(op>=99) {
     InicioEscrita(&contador_leitores, &contador_escritores, &mutex, &cond_leit);
